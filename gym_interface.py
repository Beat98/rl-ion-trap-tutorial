...
class OpenAIEnvironment(object):
    ...
    def set_up_space_simple(self, space):
        ...
        if isinstance(space, gym.spaces.Discrete):
            ...
            cardinality_list_local = space.low.size * [
                self.discretization_num_bins]  # for Box(N1,N2,...), this yields a flat list of length N1*N2*... and all entries equal to discretization_num_bins
    ...
    def observation_preprocess_simple(self, observation, discretize_observation):  # preparing a discretized observation
        ...
        if type(observation) == np.ndarray:
            observation_discretized = ((observation - discretize_observation[0]) / discretize_observation[1]).astype(
                int)  # element-wise: (raw observation - offset) / slope; casting as integers automatically acts like floor function
            observation_discretized[
                observation_discretized >= self.discretization_num_bins] = self.discretization_num_bins - 1
    ...
    def observation_preprocess_generic(self, observation):  # preparing a discretized observation
        ...
        if self.percept_space_type == 0 or self.percept_space_type == 1:
            ...
            for tuple_index in range(len(self.env.observation_space.sample())):
                observation_preprocessed += self.observation_preprocess_simple(observation[tuple_index],
                                                                               self.discretize_percepts_list[
                                                                                   tuple_index])
        return observation_preprocessed
    ...
    def action_postprocess_simple(self, action_flattened, discretize_action):
        ...
        if discretize_action == None:
            ...
            action_reshaped = np.array(action_flattened).reshape(
                discretize_action[0].shape)  # use offset array to get the right shape
            action = discretize_action[0] + (action_reshaped + 0.5) * discretize_action[
                1]  # offset + (discrete+1/2)*slope
        return action
    ...
    def action_postprocess_generic(self, action_index):
        ...
        if self.action_space_type == 0 or self.action_space_type == 1:
            ...
            for tuple_index in range(len(self.env.action_space.sample())):
                ...
                action += tuple(
                    [self.action_postprocess_simple(action_subset, self.discretize_actions_list[tuple_index])])
    ...
    def __init__(self, openai_env_name, discretization_num_bins=10):
        ...
        self.num_percepts_list, self.discretize_percepts_list, self.percept_space_type = self.set_up_space_generic(
            self.env.observation_space)
        self.num_actions_list, self.discretize_actions_list, self.action_space_type = self.set_up_space_generic(
